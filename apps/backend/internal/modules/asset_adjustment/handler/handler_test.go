...
import (
	...
	"github.com/kislikjeka/moontrack/internal/modules/asset_adjustment/handler"
	ledgerDomain "github.com/kislikjeka/moontrack/internal/core/ledger/domain"
	ledgerRepository "github.com/kislikjeka/moontrack/internal/core/ledger/repository"
...
func (m *MockLedgerRepository) CreateAccount(ctx context.Context, account *ledgerDomain.Account) error {
	args := m.Called(ctx, account)
	return args.Error(0)
}

func (m *MockLedgerRepository) GetAccount(ctx context.Context, id uuid.UUID) (*ledgerDomain.Account, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*ledgerDomain.Account), args.Error(1)
}

func (m *MockLedgerRepository) FindAccountsByWallet(ctx context.Context, walletID uuid.UUID) ([]*ledgerDomain.Account, error) {
...
func (m *MockLedgerRepository) GetAccountByCode(ctx context.Context, code string) (*ledgerDomain.Account, error) {
...
func (m *MockLedgerRepository) GetAccountBalance(ctx context.Context, accountID uuid.UUID, assetID string) (*ledgerDomain.AccountBalance, error) {
	args := m.Called(ctx, accountID, assetID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*ledgerDomain.AccountBalance), args.Error(1)
}

func (m *MockLedgerRepository) CreateTransaction(ctx context.Context, tx *ledgerDomain.Transaction) error {
	args := m.Called(ctx, tx)
	return args.Error(0)
}

func (m *MockLedgerRepository) GetTransaction(ctx context.Context, id uuid.UUID) (*ledgerDomain.Transaction, error) {
	args := m.Called(ctx, id)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*ledgerDomain.Transaction), args.Error(1)
}

func (m *MockLedgerRepository) FindTransactionsBySource(ctx context.Context, source string, externalID string) (*ledgerDomain.Transaction, error) {
...
func (m *MockLedgerRepository) ListTransactions(ctx context.Context, filters ledgerRepository.TransactionFilters) ([]*ledgerDomain.Transaction, error) {
...
func (m *MockLedgerRepository) GetEntriesByTransaction(ctx context.Context, txID uuid.UUID) ([]*ledgerDomain.Entry, error) {
	args := m.Called(ctx, txID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]*ledgerDomain.Entry), args.Error(1)
}

func (m *MockLedgerRepository) GetEntriesByAccount(ctx context.Context, accountID uuid.UUID) ([]*ledgerDomain.Entry, error) {
...
func (m *MockLedgerRepository) UpsertAccountBalance(ctx context.Context, balance *ledgerDomain.AccountBalance) error {
	args := m.Called(ctx, balance)
	return args.Error(0)
}

func (m *MockLedgerRepository) GetAccountBalances(ctx context.Context, accountID uuid.UUID) ([]*ledgerDomain.AccountBalance, error) {
	args := m.Called(ctx, accountID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
var _ ledgerRepository.LedgerRepository = (*MockLedgerRepository)(nil)
...
func TestAssetAdjustmentHandler_GenerateEntries_Balance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.balanceExists {
				mockRepo.On("GetAccountBalance", ctx, mock.AnythingOfType("uuid.UUID"), assetID).Return(&ledgerDomain.AccountBalance{
					AccountID: walletID,
					AssetID:   assetID,
			...
			ledgerSvc := service.NewLedgerService(mockRepo, nil)
			h := handler.NewAssetAdjustmentHandler(ledgerSvc)
			...
			for _, entry := range entries {
				...
				} else if entry.DebitCredit == ledgerDomain.Credit {
...
func TestAssetAdjustmentHandler_USDValueCalculation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockRepo := new(MockLedgerRepository)
			mockRepo.On("GetAccountBalance", ctx, mock.AnythingOfType("uuid.UUID"), mock.Anything).Return(&ledgerDomain.AccountBalance{
				AccountID: walletID,
				AssetID:   "ETH",
